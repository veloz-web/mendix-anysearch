define([
	...
	return declare("GridSearch.widget.Core", [_WidgetBase, _StatefulMixin], {
		...
		_setPageStart: function () {
			//override this function if the widget sets page start
		},
		_setPageEnd: function () {
			//override this function if the widget sets page end
		},
		_setHideList: function () {
			//override this function if the widget sets hide list
		},
		...
		_fireSearch: function () {
			...
			this._setHideListAllWidgets(true); // FOR SOAR
		...
		_fireSearchOneGrid: function (grid, constraints, pagesize, offset, sort) {
			...
			if (constraintsChanged || pageSizeChanged || offsetChanged || sortChanged) {
				if (grid.__customWidgetDataSourceHelper) {
					...
					self._reloadOneGrid(grid, offsetChanged, constraintsChanged, pagesize); // FOR SOAR
					console.log("set constraints for grid: " + grid.id)
			} else {
				this._setHideListAllWidgets(false); // FOR SOAR
		...
		_setOffsetAllWidgets: function (offset) {
			var searchWidgets = this._searchWidgets[this.targetGridClass];
			for (var i = 0; i < searchWidgets.length; i++) {
				searchWidgets[i]._setOffset(offset);
			}
		},
		...
		_setPageStartAndEndAllWidgets: function (start, end) {
			var searchWidgets = this._searchWidgets[this.targetGridClass];
			for (var i = 0; i < searchWidgets.length; i++) {
				searchWidgets[i]._setPageStart(start);
				searchWidgets[i]._setPageEnd(end);
			}
		},
		_setHideListAllWidgets: function (hide) {
			var searchWidgets = this._searchWidgets[this.targetGridClass];
			for (var i = 0; i < searchWidgets.length; i++) {
				searchWidgets[i]._setHideList(hide);
			}
		},
		...
		_reloadGrid: function (forceOffset, constraintsChanged, pagesize) { // FOR SOAR
			for (var i = 0; i < this._grids.length; i++) {
				this._reloadOneGrid(this._grids[i], forceOffset, constraintsChanged, pagesize); // FOR SOAR
			}
		},
		_reloadOneGrid: function (grid, forceOffset, constraintsChanged, pagesize) { // FOR SOAR
			//if this grid has List View Controls connected, use its loader instead
			const callback = () => {
				grid.__customWidgetPagingLoading = false;
				this._skipProgressBar(false); // FOR SOAR
				this._setTotalCount(grid, constraintsChanged, pagesize); // FOR SOAR
			}
			if (grid.reload) {
				this._skipProgressBar(true); // FOR SOAR
		...
		// FOR SOAR
		_skipProgressBar: function (start) {
		...
		_setTotalCount: function(grid, constraintsChanged, pagesize) { // FOR SOAR
			if(this.countAttribute) {
				...
				// FOR SOAR
				if(datasource._setSize === 0) {
					this._setPageStartAndEndAllWidgets(0, 0);
				}
				else if(constraintsChanged) {
					if(pagesize == null || pagesize > datasource._setSize) {
						this._setPageStartAndEndAllWidgets(1, datasource._setSize);
					}
					else {
						this._setPageStartAndEndAllWidgets(1, pagesize);
					}
				}
			}
			this._setHideListAllWidgets(false); // FOR SOAR
